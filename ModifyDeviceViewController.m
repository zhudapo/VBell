...
@interface ModifyDeviceViewController()<UITextFieldDelegate>

@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, strong) UIView *container;
@property (nonatomic, strong) NSArray *modifyDeviceArray;
...
- (void)viewDidLoad {
    ...
    NSString *modifyAccount = NSLocalizedString(@"Modify Device", nil);
    NSString *strTitle = _name.length==0? [NSString stringWithFormat:@"%@[Account 1]",modifyAccount]:[NSString stringWithFormat:@"%@ [%@]",modifyAccount,_name];
    self.title = strTitle;
    
    
    //查询显示数据
    VBellsqlBase *base = [VBellsqlBase shareMyDdataBase];
    self.modifyDeviceArray = [base getUserInfoNameWithNumber:_number];
    userModel *model = nil;
    if (_modifyDeviceArray.count != 0) {
        model = _modifyDeviceArray[0];
    }else{
        return;
    }
    self.unlockSwitch = [[UISwitch alloc]init];
    self.videoPreviewSwitch = [[UISwitch alloc]init];
    ...
    self.DTMFLTxt = [[UITextField alloc]init];
    _DTMFLTxt.text = model.DTMF;
    ...
    if (model.unLock == 0) {
        [_unlockSwitch setOn:NO];
        
    }else{
        [_unlockSwitch setOn:YES];
        [self callFeaturesChanged:_unlockSwitch];
    }
    if (model.videoPreview == 0) {
        [_videoPreviewSwitch setOn:NO];
        
    }else{
        [_videoPreviewSwitch setOn:YES];
        [self callFeaturesChanged:_videoPreviewSwitch];
    }
}
...
- (UIView *)setupCallFeaturesPart
{
    ...
    self.unlockSwitch.frame = CGRectMake(view.bounds.size.width-50, marginHeight + 23, 50, labelHeight);
    self.videoPreviewSwitch.frame = CGRectMake(view.bounds.size.width-50, marginHeight + 23 + labelHeight, 50, labelHeight);
...
- (UIView *)setupFooterPart
{
    ...
    [_saveButton addTarget:self action:@selector(saveModify:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIView *)setupCallFeaturesPartChanged
{
    ...
    self.DTMFLTxt.frame = CGRectMake(view.bounds.size.width*3/4, marginHeight  + labelHeight + 23, view.bounds.size.width/4, labelHeight);
...
- (void)saveModify:(UIButton *)btn
{
    ...
    if (_displayName.text.length == 0) {
        NSLog(@"Device name needed");
        return;
    }else if (_deviceNumber.text.length == 0){
        NSLog(@"Device number needed");
        return;
    }else {
        NSString *address = nil;
        VBellsqlBase *base = [VBellsqlBase shareMyDdataBase];
        if (_rtspTxt.text.length == 0) {
            address = _deviceNumber.text;
        }else{
            _rtsp = _rtspTxt.text;
        }
        //变更本地数据库
        BOOL reslut = [base updateDeviceName:_displayName.text oldNumber:_number rtspAddress:_rtsp newNumber:_deviceNumber.text unlock:unlockStatus dtmf:dtmf video_preview:void_previewStatus];
        if (reslut) {
            if ([_RTSPSwitch isOn]) {
                
                if ([self.delegate respondsToSelector:@selector(changeDeviceInfoRTSPStatus:andShowRtspLable:)]) {
                    //此版本注释RTSP功能设置
//                    [self.delegate changeDeviceInfoRTSPStatus:YES andShowRtspLable:_rtspTxt.text];
                }
            }else{
                
                if ([self.delegate respondsToSelector:@selector(changeDeviceInfoRTSPStatus:andShowRtspLable:)]) {
                    //此版本注释RTSP功能设置
//                    [self.delegate changeDeviceInfoRTSPStatus:NO andShowRtspLable:_rtspTxt.text];
                }
            }
            //传递的值
            NSMutableDictionary *mutableDictionary = [NSMutableDictionary dictionaryWithObject:_deviceNumber.text forKey:@"deviceNumber"];
            [mutableDictionary setValue:_displayName.text forKey:@"deviceName"];
            //发出通知
            [[NSNotificationCenter defaultCenter]postNotificationName:@"MODIFYDEVICE" object:mutableDictionary];
            
            [base updateCallHistoryInfoTab:_displayName.text oldNumber:_number newNumber:_deviceNumber.text];
            
            MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
            hud.mode = MBProgressHUDModeText;
            hud.label.text = NSLocalizedString(@"Modify Device Success!", @"HUD message title");
            hud.offset = CGPointMake(0.f, 0.f);
            [hud hideAnimated:YES afterDelay:1.f];
        }else{
            MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
            hud.mode = MBProgressHUDModeText;
            hud.label.text = NSLocalizedString(@"Modify Device Failed!", @"HUD message title");
            hud.offset = CGPointMake(0.f, 0.f);
            [hud hideAnimated:YES afterDelay:1.f];
        }
